...
class TestEncryptionMode(unittest.TestCase):
    def test_mode(self):
        ...
        for k in range(4):
            self.assertEquals(len([i for i, j in zip(test_data.test_ofb_ciphertext[k],
                                                     test_ofb.encrypt_block(test_data.test_mode_plaintext[k])) if
                                   i == j]),
                              16,
        ...
        for k in range(4):
            self.assertEquals(len([i for i, j in zip(test_data.test_mode_plaintext[k],
                                                     test_ofb.decrypt_block(test_data.test_ofb_ciphertext[k])) if
                                   i == j]),
                              16,
